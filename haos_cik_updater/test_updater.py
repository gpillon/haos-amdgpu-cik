...
class UpdaterTests(unittest.TestCase):
    ...
    def test_reads_installed_version_with_legacy_hassio_token(self) -> None:
        response = Response(
            json.dumps({"result": "ok", "data": {"version": "18.3.20260920013543"}}).encode()
        )
        with (
            ...
            self.assertEqual(updater.supervisor_os_version(), "18.3.20260920013543")
        request = urlopen.call_args.args[0]
        self.assertEqual(request.headers["Authorization"], "Bearer legacy-token")

    def test_reboots_with_legacy_hassio_token(self) -> None:
        response = Response(b"")
        response.status = 200
        with (
            mock.patch.dict(
            ...
            updater.State._reboot_host()
        request = urlopen.call_args.args[0]
        self.assertEqual(request.full_url, "http://supervisor/host/reboot")
        self.assertEqual(request.headers["Authorization"], "Bearer legacy-token")
